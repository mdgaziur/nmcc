...
LexicalToken *maybe_handle_space(Lexer *lexer, LexicalToken *token,
                                 NMVec *diagnostic) {
  if (lexer->handle_whitespace) {
    ...
    token->span =
        span_for_single_char(lexer->file_path, lexer->cur_line, lexer->cur_col);
 ...
 */
void lex_upto_two_char_token(Lexer *lexer, LexicalToken *token,
                             LexKind single_kind, const char *other_chars,
                             const LexKind *other_kinds, size_t n_other_kinds,
  ...
  for (cur_token = 0; cur_token < n_other_kinds; cur_token++) {
    if (next_char == other_chars[cur_token]) {
      ...
      token->span = span_new(lexer->file_path, line_start, col_start,
...
void lex_string(Lexer *lexer, LexicalToken **token, bool is_wide_string,
                NMVec *diagnostics) {
  ...
  if (!lexer->cur_char) {
    ...
    nmvec_push(diagnostics, &(Diagnostic *){diagnostic_for_single_char(
                                DIAG_ERROR, lexer->file, "Unexpected EOF",
                                lexer->cur_line, lexer->cur_col)});
  } else if (*lexer->cur_char != '"') {
    has_error = true;

    nmvec_push(diagnostics, &(Diagnostic *){diagnostic_for_single_char(
                                DIAG_ERROR, lexer->file, "Unterminated string",
                                lexer->cur_line, lexer->cur_col)});
  }
...
void lex_float_or_decimal(Lexer *lexer, LexicalToken **token,
                          NMVec *diagnostics) {
  NMString *lexeme = nmstring_new_from_char(*lexer->cur_char);
  ...
  if (dot_count > 1) {
    ...
    nmvec_push(diagnostics,
               &(Diagnostic *){diagnostic_for_span(
                   DIAG_ERROR, "Invalid floating constant", lexer->file, &sp)});
...
void lex_identifier(Lexer *lexer, LexicalToken *token) {
  ...
  while (*lexer->cur_char &&
         (isalnum(*lexer->cur_char) || *lexer->cur_char == '_')) {
    nmstring_append(lexeme, *lexer->cur_char);
    LEXER_ADVANCE(1);
  }
  ...
  if (NM_EQ_C(lexeme, "auto"))
    token->kind = LEX_AUTO;
  else if (NM_EQ_C(lexeme, "double"))
    token->kind = LEX_DOUBLE;
  else if (NM_EQ_C(lexeme, "int"))
    token->kind = LEX_INT;
  else if (NM_EQ_C(lexeme, "struct"))
    token->kind = LEX_STRUCT;
  else if (NM_EQ_C(lexeme, "break"))
    token->kind = LEX_BREAK;
  else if (NM_EQ_C(lexeme, "else"))
    token->kind = LEX_ELSE;
  else if (NM_EQ_C(lexeme, "long"))
    token->kind = LEX_LONG;
  else if (NM_EQ_C(lexeme, "switch"))
    token->kind = LEX_SWITCH;
  else if (NM_EQ_C(lexeme, "case"))
    token->kind = LEX_CASE;
  else if (NM_EQ_C(lexeme, "enum"))
    token->kind = LEX_ENUM;
  else if (NM_EQ_C(lexeme, "typedef"))
    token->kind = LEX_TYPEDEF;
  else if (NM_EQ_C(lexeme, "char"))
    token->kind = LEX_CHAR;
  else if (NM_EQ_C(lexeme, "extern"))
    token->kind = LEX_EXTERN;
  else if (NM_EQ_C(lexeme, "return"))
    token->kind = LEX_RETURN;
  else if (NM_EQ_C(lexeme, "union"))
    token->kind = LEX_UNION;
  else if (NM_EQ_C(lexeme, "const"))
    token->kind = LEX_CONST;
  else if (NM_EQ_C(lexeme, "float"))
    token->kind = LEX_FLOAT;
  else if (NM_EQ_C(lexeme, "short"))
    token->kind = LEX_SHORT;
  else if (NM_EQ_C(lexeme, "unsigned"))
    token->kind = LEX_UNSIGNED;
  else if (NM_EQ_C(lexeme, "continue"))
    token->kind = LEX_CONTINUE;
  else if (NM_EQ_C(lexeme, "for"))
    token->kind = LEX_FOR;
  else if (NM_EQ_C(lexeme, "signed"))
    token->kind = LEX_SIGNED;
  else if (NM_EQ_C(lexeme, "void"))
    token->kind = LEX_VOID;
  else if (NM_EQ_C(lexeme, "default"))
    token->kind = LEX_DEFAULT;
  else if (NM_EQ_C(lexeme, "goto"))
    token->kind = LEX_GOTO;
  else if (NM_EQ_C(lexeme, "sizeof"))
    token->kind = LEX_SIZEOF;
  else if (NM_EQ_C(lexeme, "volatile"))
    token->kind = LEX_VOLATILE;
  else if (NM_EQ_C(lexeme, "do"))
    token->kind = LEX_DO;
  else if (NM_EQ_C(lexeme, "if"))
    token->kind = LEX_IF;
  else if (NM_EQ_C(lexeme, "static"))
    token->kind = LEX_STATIC;
  else if (NM_EQ_C(lexeme, "while"))
    token->kind = LEX_WHILE;
  else
...
#define SINGLE_CHAR_TOKEN(tok, tok_kind)                                       \
  case tok: {                                                                  \
    token->kind = tok_kind;                                                    \
    is_single_char_token = true;                                               \
    break;                                                                     \
...
#define TWO_CHAR_TOKEN(tok, single_kind, other_chars, ...)                     \
  case tok: {                                                                  \
    const LexKind kinds[] = {__VA_ARGS__};                                     \
    lex_upto_two_char_token(lexer, token, single_kind, other_chars, kinds,     \
...
#define EXPECT(ch)                                                             \
  if (*lexer->cur_char != ch) {                                                \
    nmvec_push(diagnostics,                                                    \
               &(Diagnostic *){diagnostic_for_single_char(                     \
                   DIAG_ERROR, lexer->file, "Invalid syntax for `#line`",      \
                   lexer->cur_line, lexer->cur_col)});                         \
    lexical_token_free(token);                                                 \
    token = NULL;                                                              \
    is_single_char_token = false;                                              \
  ...
  if (!*lexer->cur_char) {
    token->kind = LEX_EOF;
    token->span = span_new(lexer->file_path, lexer->cur_line, lexer->cur_col,
                           lexer->cur_line, lexer->cur_col);
    return token;
  }
  const char next_char = *(lexer->cur_char + 1);
...
void debug_lexical_token(LexicalToken *token) {
  ...
  fprintf(stderr, "\tspan: %s:%zu:%zu-%zu:%zu\n", token->span.file_path,
          token->span.line_start, token->span.linepos_start,
          token->span.line_end, token->span.linepos_end);
  fprintf(stderr, "}\n");
}

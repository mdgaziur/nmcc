...
bool should_emit_code() {
    return preprocessor_state.should_emit;
}
...
NMString *eat_ident(const char **line_ptr) {
    ...
    while (**line_ptr && (isalpha(**line_ptr) || **line_ptr == '_')) nmstring_append(ident, *((*line_ptr)++));
...
NMString *collect_define_macro_body(const char **line_ptr, bool *has_extra_line) {
...
void preprocess_define(const char *line_ptr) {
    ...
    preprocessor_state.macros = realloc(preprocessor_state.macros, ++preprocessor_state.n_macros * sizeof(MacroDefine*));
    NOT_NULL(preprocessor_state.macros, "Failed to allocate memory for macro definition");
...
void preprocess_include(const char *line_ptr, const char *file_path, NMString *preprocessed_code) {
    ...
    while (*line_ptr && *line_ptr != end_char) nmstring_append(path, *line_ptr++);
    ...
    for (i = 0; i < preprocessor_state.n_include_directories && !has_found; i++) {
        NMString *search_path = nmstring_new_from_str(preprocessor_state.include_directories[i].buf);
        nmstring_append(search_path, '/');
        nmstring_append_nmstring(search_path, path);
...
void preprocess1_add_include_directory(NMString *path) {
    ...
    NOT_NULL(preprocessor_state.include_directories, "Failed to reallocate buffer to hold include_dirs");

    preprocessor_state.include_directories[preprocessor_state.n_include_directories] = *path;
...
void preprocess1_code(NMFile *src, NMString *preprocessed_code) {
    ...
    while (getline(&line, &len, src_file) != -1) {
        ...
        if (preprocessor_state.append_line_to_last_macro_def) {
            ...
            nmstring_append_nmstring(preprocessor_state.macros[preprocessor_state.n_macros - 1]->body, body);
            preprocessor_state.append_line_to_last_macro_def = has_extra_line;
        } else if (*ptr == '#') {
            ptr++;
            ...
            while (*ptr && (isalpha(*ptr) || *ptr == '_')) nmstring_append(directive, *ptr++);
            ...
            for (i = 0; i < strlen(line); i++) {
                ...
                if (isalpha(line[i]) || line[i] == '_') {
                        ...
                        if (!strcmp(S(preprocessor_state.macros[j]->name), S(cur_ident))) {
                            ...
                            if (macro_defn->is_function_like) {
                                ...
                                for (cur_arg = 0; cur_arg < macro_defn->n_args; cur_arg++) {
                                    ...
                                    nmstring_replace(res_body, S(macro_defn->args[cur_arg]), S(arg_val));
                                }
                    ...
                    if (!is_macro) nmstring_append_nmstring(preprocessed_code, cur_ident);
                    nmstring_append(preprocessed_code, line[i]);
